...
def parse_args():
    ...
    parser.add_argument("js", help="Json file")
...
def main(args):
    ...
    js = json.load(open(os.path.join(args.js)))
    imgs = js['images']
    for img in imgs:
        id = img['id']
        img = args.img + img['file_name']
        # test a single image
        ...
        #         ann['bbox'] += [1]
        #         bbox = np.array(ann['bbox']).reshape([1, 5])
        #         result = np.concatenate((result, bbox), axis=0)
        # result = [result]
        # show the results
