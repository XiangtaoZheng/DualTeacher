...
data = dict(
    ...
    train=dict(
        ...
        sup2=dict(
            type="HRSIDDataset",
            ann_file="data/ssdd/annotations/semi_supervised/instances_train2017.${fold}@${percent}.json",
            img_prefix="data/ssdd/JPEGImages/",
        ),
        unsup=dict(
            type="HRSIDDataset",
            ann_file="data/ssdd/annotations/semi_supervised/instances_train2017.${fold}@${percent}-unlabeled.json",
            img_prefix="data/ssdd/JPEGImages/",
        ),
    ),
    val=dict(
...
semi_wrapper = dict(
    train_cfg=dict(
        unsup_weight=2.0,
        load1_from='work_dirs/faster_rcnn_r101_caffe_fpn_dior_full_180k/100/${fold}/iter_1000.pth',
        load2_from='work_dirs/faster_rcnn_r101_caffe_fpn_dior_ssdd_full_180k/${percent}/${fold}/iter_1000.pth',
    ),
